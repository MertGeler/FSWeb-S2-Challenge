...
function cumlelereDonustur(cumleler, ayrac = ",") {
  const yeniCumleler = cumleler.map((kelimeler) => kelimeler.join(ayrac));

  return yeniCumleler;
}
const sonuclar = cumlelereDonustur(cumleler, " ");
console.log("GÖREV 1:", sonuclar);
...
//3a çözümü
/* kodlar buraya */
const ilkMeyve = meyveler.shift();
const sonMeyve = meyveler.pop();

console.log("Çıkarılan İlk Meyve:", ilkMeyve);
console.log("Çıkarılan Son Meyve:", sonMeyve);
console.log("Güncel Meyveler Dizisi:", meyveler);
...
console.log("Güncellenen Sebzeler Dizisi:", sebzeler);
...
function emojileriDonustur(mesaj, emojiler) {
  for (const sembol in emojiler) {
    const buyukHarfSembol = sembol.toUpperCase();
    const kucukHarfSembol = sembol.toLowerCase();

    mesaj = mesaj.replaceAll(buyukHarfSembol, emojiler[sembol]);
    mesaj = mesaj.replaceAll(kucukHarfSembol, emojiler[sembol]);
  }
...
const dönüştürülmüsMesaj = emojileriDonustur(mesaj, emojiler);
console.log("GÖREV 4:", dönüştürülmüsMesaj);
